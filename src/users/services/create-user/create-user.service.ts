...
export class CreateUserService extends Assertion implements UserService {
	...
	constructor(@Inject("USER_REPOSITORY") private readonly repository: UserRepositoty) {
		super()
	}
	...
	async _validarLoginDuplicado(login) {
		const userWithSameLogin = await this.repository.findByLogin(login)
			.then(res => res)
			.catch(err => console.log(err));

		this.assertFalse(userWithSameLogin,
