...
import { NotFoundError } from 'src/common/exceptions/NotFoundError';
...
export class MongoUserRepository implements UserRepositoty {
  constructor(
    @InjectModel('USER_MODEL') private readonly mongoUser: Model<any>,
  ) { }
  ...
  async save(novoUsuario) {
    ...
    const user = await this.mongoUser
      .findOneAndUpdate(
        { _id: id },
        {
          $set: {
  ...
  async findByLogin(login) {
    const usuario = await this.mongoUser
      .findOne({ login }, '-__v')
      .then((res) => res);
